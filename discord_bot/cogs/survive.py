...
def generate_situation(used):
    ...
    prompt = """Tum Discord multiplayer decision game ke expert question designer ho.

Har baar ek bilkul naya, interesting aur unpredictable Hindi/Hinglish situation banao.

Topics sirf survival/disaster tak limited nahi hone chahiye. In topics ko mix karo:
mystery, logic, social dilemma, betrayal, negotiation, exploration, strategy,
technology, crime mystery, unusual events, moral dilemma, psychological situations,
competition aur survival.

Question EASY nahi hona chahiye. Player ko 3 options compare karke genuinely sochna pade.

IMPORTANT:
- Fixed phrase "ka khatra kareeb aa raha hai" bilkul mat use karo.
- Situation 1-3 sentences ki ho.
- Natural Hindi/Hinglish me likho.
- Har baar previous situations se clearly different idea do.
- Generic options jaise "Plan", "Run", "Fight", "Hide" mat do.
- Exactly 3 context-specific actions do.
- Teeno actions believable hon aur unke consequences alag feel hon.
- Emoji har action ke saath do.

JSON ke EXACT format me answer do:
{
  "situation": "situation text",
  "actions": [
    {"emoji": "emoji1", "label": "action 1"},
    {"emoji": "emoji2", "label": "action 2"},
    {"emoji": "emoji3", "label": "action 3"}
  ]
}
"""
    ...
    for _ in range(3):
        try:
            ...
            if len(actions) != 3:
                raise ValueError("AI did not return exactly 3 actions")
            ...
            key = (situation, tuple(round_choices))

            if key not in used:
                used.add(key)
                return situation, round_choices

        except Exception as e:
            print("AI generator error:", e)

    return "Ek unexpected situation saamne aa gayi hai. Tum kya karoge?", [
        ("🧠", "Situation ko dhyan se analyze karo"),
        ("🔎", "Kuch unusual clue investigate karo"),
        ("🤝", "Baaki players se strategy discuss karo")
    ]
...
class Survive(commands.Cog):
        ...
        if action.value == "close":
            ...
            if not game:
                return await interaction.response.send_message(
                    "No active SURVIVE game.",
                    ephemeral=True
                )

            if interaction.user.id != game.host_id:
                return await interaction.response.send_message(
                    "Only the host can close the SURVIVE game.",
                    ephemeral=True
                )
    ...
    async def start_game(self, interaction, gid):
        ...
        if len(game.players) < 2:
            return await interaction.response.send_message(f"Need at least 2 players. Current: {len(game.players)}.", ephemeral=True)
        game.started = True
    ...
    async def new_round(self, channel, game):
        ...
        game.round_choices = round_choices
    ...
    async def choose(self, interaction, gid, round_no, choice):
        ...
        emoji, label = game.round_choices[choice]
        await interaction.response.send_message(
            f"🔒 Locked: **{emoji} {label}**",
            ephemeral=True
        )
